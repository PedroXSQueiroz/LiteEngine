...
Branch `abstract_scene`: abstração em andamento. `main.cpp` ainda é um sandbox que carrega IBL + modelo FBX hardcoded (com `deepIds=true`), monta uma UI de teste (input de path + botões Carregar/Deletar), roda câmera FPS inline e faz **picking por clique** (`FilamentObjectSelectorSystem` + `getCurrentCamera()` do renderer; o mesh clicado ganha wireframe de seleção via `postCommand`); o wireframe rastreia meshes automaticamente (auto-track). Objetivo declarado: mover tudo para facades/interfaces (main loop dentro da Scene, serviços de materiais/assets, física Jolt, animação ozz).

As refatorações de julho/2026 (thread affinity da UI, fases do `SceneRenderer`, lifecycle de systems, ids por nó) e o picking foram **validados em runtime** (2026-07-20): cena + UI + wireframe + seleção funcionam, sem PanicLog. Backlog de refinos pendentes (threading do picking, hit mais próximo, backface culling, índice id→nó etc.) está na memória do projeto (`perf-architecture-backlog`).
