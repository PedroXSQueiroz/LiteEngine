...
namespace lite{
    ...
    class UIComboBoxInputElement : public UIElement<URT>{
        ...
        virtual bool getOptionSelected(std::string key) = 0;
    ...
    protected:
        ...
        void notifyChange(){
            std::string currentSelectedOption = this->getOptionSelected(); 
            
            for(std::function<void(UIComboBoxInputElement*)> currentCallback : this->onSelectValueChange){
                currentCallback(currentSelectedOption);
            }
        };

        virtual bool updateInput(std::string key);
